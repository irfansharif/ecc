...
VALUE private_key_verify(VALUE self, VALUE private_key) {
  char* private_key_buffer = StringValuePtr(private_key);
...
VALUE private_key_tweak_add(VALUE self, VALUE private_key, VALUE tweak) {
  ...
  memcpy(private_key_buffer, StringValuePtr(private_key), 32);
  char* tweak_buffer = StringValuePtr(tweak);
  ...
  return rb_str_new(private_key_buffer, 32);
}
